...
#     return (im - np.nanmean(im, axis)) / (np.nanstd(im, axis) + c)
    return im#(im - im.mean(axis)) / (im.std(axis) + c)
...
class FrameInfo:
    ...
    def __init__(self, img, annotations,dtype=np.float32):# weight, 
...
#         self.weight = weight
    ...
    def getPatch(self, i, j, patch_size, img_size, normalize=0):  #1.0
...
#         print(im.shape, an.shape)
        comb_img = np.concatenate((im, an), axis=-1) #, we
